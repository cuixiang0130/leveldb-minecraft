...
inline bool Zlib_Compress(const char* input, size_t length,bool raw, std::string & output) {
#if HAVE_ZLIB
  ...
  auto res = deflateInit2(&strm, Z_DEFAULT_COMPRESSION, Z_DEFLATED,
  ...
  if (deflate_res != Z_STREAM_END) return false;
...
inline bool Zlib_Uncompress(const char* input, size_t length,bool raw, std::string& output) {
#if HAVE_ZLIB
